...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            echo "<script>alert('Login successful!'); window.location.href = 'index.php';</script>"; // Redirect to home page
        } else {
            echo "<script>alert('Incorrect password!');</script>";
        }
    } else {
        echo "<script>alert('No user found with this email and username combination!');</script>";
    }
}
?>
...
<body>
        ...
        <div class="collapse navbar-collapse hamburgeritems enddiv order-3">
            <ul class="navbar-nav end-links">
                ...
                <?php else: ?>
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="signup.php"><i class="fa-solid fa-user"></i> Sign-Up</a>
                    </li>
                <?php endif; ?>
                <li class="nav-item">
                ...
                <form method="POST" action="login.php" id="login-form">
                    <div class="card-body">
                        ...
                        <div class="row justify-content-between">
                            <div class="col">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" value="" id="rememberCheckbox" checked>
                            ...
                            <div class="col">
                                <a href="#forgot-pwd" data-bs-toggle="modal" role="button">
                                    <p>Forgot Password?</p>
                                </a>
                            </div>
                        </div>
                        <div class="text-center">
                            ...
                            <button id="google-login" class="btn-login" type="submit">Login with Google</button>
                        </div>
                        <h1 class="text-center">Don't have an account?
	...
	<div class="modal fade" id="forgot-pwd" aria-hidden="true" tabindex="-1" aria-labelledby="forgot-pwd">
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content container p-4">
				<form id="pwdforget-form" class="needs-validation" novalidate onsubmit="return false">
					<h1 class="title text-center">Forgot Password</h1>
					<div class="textfield">
						<label for="forgot-email">Email</label>
						<input type="email" name="email" id="forgot-email" placeholder="Enter Email" required>
						<div class="invalid-feedback">
							Enter a valid email address!
						</div>
					</div>
					<div class="text-center">
						<button class="btn-login pwdforget-submit-btn" type="submit">Submit</button>
					</div>
				</form>
			</div>
		</div>
	</div>
	<div class="modal fade" id="submit-success" aria-hidden="true" aria-labelledby="submit-success" tabindex="-1">
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content container p-4 text-center">
				<h1 class="title text-center">Great</h1>
	...
	<script>
		...
		(() => {
			'use strict'
			const form = document.getElementById("pwdforget-form");
			const firstmodal = new bootstrap.Modal('#forgot-pwd');

			firstmodal._element.addEventListener("show.bs.modal", (event) => {
				form.reset();
			...
			form.addEventListener('submit', event => {
				...
				const submitbtn = document.getElementsByClassName("pwdforget-submit-btn")[0];
				if(!form.checkValidity())
				{
					...
					submitbtn.setAttribute("data-bs-target", "#submit-success");
					submitbtn.click();
					submitbtn.removeAttribute("data-bs-toggle");
					submitbtn.removeAttribute("data-bs-target");
				}
